...
2. Draw a rectangle with the Add Rectangles tool ![](manual/assets/icons/rectangle.svg){width="24"} (selected by default)
...
- - `deduplicate masks`
  - Enables removal of duplicate masks.
    ...
    For details, see the [duplicate mask removal appendix](appendix.qmd).
  - **True**
...
If particles were missed, increase the label number in the upper-left panel by one, then draw a new particle mask using the paint tool ![](manual/assets/icons/paint.svg){width="24"}.
You can adjust brush thickness via brush size.

### When you want to split particles

If multiple particles are connected, erase one side with the eraser tool ![](manual/assets/icons/erase.svg){width="24"}, then increase the label number and repaint a new particle mask.

### When you want to erase a wide area

If background is falsely detected as a mask, click an empty area with the picker tool ![](manual/assets/icons/picker.svg){width="24"} to select background, then click the background mask with fill tool ![](manual/assets/icons/fill.svg){width="24"} to erase in bulk.

Similarly, to erase particles in chunks, pick the particle color with the picker tool ![](manual/assets/icons/picker.svg){width="24"} and then click that particle with fill tool ![](manual/assets/icons/fill.svg){width="24"} to erase in bulk.

### When you want to remove falsely detected particles

If noise (e.g., dust) is falsely detected as particles, erase it with the eraser tool ![](manual/assets/icons/erase.svg){width="24"}.
...
After export is complete, run **Delete all layers for current image** to remove **all layers currently loaded in the viewer** (not only the current image).
A confirmation dialog appears, but this operation can discard unrelated in-progress work, so use it with care.
...
Some interface icons used in this manual (`fill.svg`, `paint.svg`, `erase.svg`, `picker.svg`, and `rectangle.svg`) are copied and/or adapted from the [napari](https://github.com/napari/napari) project.

Canonical source: [`src/napari/resources/icons` @ `c3e6c4ab549eaeb67c6f98502851dd99e4804522`](https://github.com/napari/napari/tree/c3e6c4ab549eaeb67c6f98502851dd99e4804522/src/napari/resources/icons).
These icon files are copyright (c) Napari and/or napari contributors and are redistributed under the [BSD 3-Clause License](https://github.com/napari/napari/blob/c3e6c4ab549eaeb67c6f98502851dd99e4804522/LICENSE). See [`LICENSES/napari-BSD-3-Clause.txt`](LICENSES/napari-BSD-3-Clause.txt) for the full license text.
